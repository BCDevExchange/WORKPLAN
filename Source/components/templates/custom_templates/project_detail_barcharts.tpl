    ...
    <script type="text/javascript">
      ...
      function drawChart() {
          ...
          ['Name','Spent','Approved'],
			{/literal}
				{foreach item=Row from=$DataGrid.Rows name=RowsGrid}
					['Budget', {$Row.DataCells.budget_spent.Data},{$Row.DataCells.approved_budget.Data}],
				{/foreach}
			{literal}
        ]);	
		...
		var options1 = {
          title: {/literal}
				{foreach item=Row from=$DataGrid.Rows name=RowsGrid}
				'{$Row.DataCells.project_name.Data} - Budget'
				{/foreach} 
			{literal},
        ...
        {/literal}
			{foreach item=Row from=$DataGrid.Rows name=RowsGrid}
				['Hours', {$Row.DataCells.total_hours.Data}, {$Row.DataCells.assigned_hours.Data}],
			{/foreach}
        {literal}
        ]);
		var options2 = {
          title: {/literal}
				{foreach item=Row from=$DataGrid.Rows name=RowsGrid}
				'{$Row.DataCells.project_name.Data} - Hours'
				{/foreach} 
			{literal},
			legend: 'bottom',
			colors:['green','orange']
        ...
        var data3 = google.visualization.arrayToDataTable([
          ['Project', 'Actual',{ role: 'tooltip' }, 'Total', { role: 'tooltip' }],
                     {/literal}
                           {foreach item=Row from=$DataGrid.Rows name=RowsGrid}
                                  ['Progress', {$Row.DataCells.progress.Data},{$Row.DataCells.progress.Data}*100+'%',{$Row.DataCells.assigned_progress.Data}, '100%']
                           {/foreach}
                     {literal}
        ]);
		...
		var options3 = {
          title: {/literal}
				{foreach item=Row from=$DataGrid.Rows name=RowsGrid}
				'{$Row.DataCells.project_name.Data} - Percent Done'
				{/foreach} 
			{literal}, 
			legend: 'bottom',
			colors:['green','orange'],
